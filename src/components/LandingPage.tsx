...
function ProductTile({ card }: { card: ProductCard }) {
  return (
    <a href={card.linkUrl} className="card-wrapper">
      <div className="card__media ratio" style={ratioStyle(card.imageRatio)}>
        ...
        {!reverse ? (
          <>
            <a href={largeCard.linkUrl} className="card-wrapper collage__item--large">
              <div className="card__media ratio" style={ratioStyle(largeCard.imageRatio)}>
                <img src={largeCard.imageUrl} alt={largeCard.title} />
              </div>
            </a>
...
export function LandingPage({ content }: LandingPageProps) {
  ...
  const slides = useMemo<HeroSlide[]>(() => content.heroSlides, [content.heroSlides]);
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      setActiveSlide((current) => {
        ...
        // If current slide is a video and it's not ended, don't advance
        if (currentSlide.mediaType === "video" && !videoEnded[currentSlide.id] && !pausedVideos[currentSlide.id]) {
          return current;
        }
  ...
  }, [slides.length, videoEnded, pausedVideos, slides]);
  ...
  useEffect(() => {
    slides.forEach((slide, index) => {
      ...
      const handleVideoEnd = () => {
        setVideoEnded(prev => ({ ...prev, [slide.id]: true }));
      };

      const handleVideoPlay = () => {
        setVideoEnded(prev => ({ ...prev, [slide.id]: false }));
      };
      ...
      if (index === activeSlide && !pausedVideos[slide.id]) {
        video.currentTime = 0; // Reset video to start
        video.muted = mutedVideos[slide.id] ?? true; // Default to muted
  ...
  }, [activeSlide, pausedVideos, mutedVideos, slides]);
  ...
  const toggleVideo = (slideId: string) => {
    ...
    setPausedVideos((current) => {
      const nextPaused = !current[slideId];
  ...
  const toggleMute = (slideId: string) => {
    const video = videoRefs.current[slideId];
    if (!video) {
      return;
    }
    ...
    setMutedVideos((current) => {
      const nextMuted = !current[slideId];
  ...
  return (
    <main id="frontend-view">
      ...
      <section className="slideshow">
        {slides.length > 1 ? (
          <button type="button" className="slide-arrow slide-arrow--right" onClick={showNextSlide} aria-label="Next slide">
        ...
        {slides.map((slide, index) => {
          ...
          const isPaused = pausedVideos[slide.id];
          const isMuted = mutedVideos[slide.id] ?? true;

          return (
            <div key={slide.id} className={`slide ${isActive ? "active" : ""}`}>
              {slide.mediaType === "video" ? (
                <>
                  <video
                    ...
                    muted={isMuted}
                    playsInline
                    style={slide.aspectRatio ? { aspectRatio: slide.aspectRatio } : {}}
                  />
                    ...
                    >
                      {isPaused ? (
                        <svg width="72" height="72" viewBox="0 0 72 72" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M30 35.9999V25.6079L39 30.8039L48 35.9999L39 41.1959L30 46.3919V35.9999Z" fill="white"/>
                        </svg>
                      ) : (
                        <svg width="72" height="72" viewBox="0 0 72 72" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <rect x="28" y="25" width="6" height="22" fill="white"/>
                          <rect x="38" y="25" width="6" height="22" fill="white"/>
                        </svg>
                      )}
                    </button>
                    <button
                      type="button"
                      className="video-controls mute-unmute"
                      onClick={() => toggleMute(slide.id)}
                      aria-label={isMuted ? "Unmute video" : "Mute video"}
                    >
                      {isMuted ? (
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M16.5 12C16.5 10.23 15.48 8.71 14 7.97V9.18L16.45 11.63C16.48 11.86 16.5 12.08 16.5 12ZM19 12C19 12.94 18.8 13.82 18.46 14.64L19.97 16.15C20.63 14.91 21 13.5 21 12C21 7.72 18 4.14 13.5 3.23V5.29C16.89 6.15 19 8.83 19 12ZM4.27 3L3 4.27L7.73 9H3V15H7L12 20V13.27L16.25 17.53C15.58 18.04 14.83 18.46 14 18.7V20.77C15.38 20.45 16.63 19.82 17.68 18.96L19.73 21L21 19.73L12 10.73L4.27 3ZM12 4L9.91 6.09L12 8.18V4Z" fill="white"/>
                        </svg>
                      ) : (
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <img src={slide.mediaUrl} alt={`Slide ${index + 1}`} />
              )}
              {slide.buttonText ? (
        ...
        <div className="grid grid--4-col">
          {content.collections.map((collection) => (
            <a key={collection.id} href={collection.linkUrl} className="card-wrapper">
              <div className="card__media ratio" style={ratioStyle("125%")}>
